...
import type { PhilosophyCard, CategoryItem, SubCard } from '@/lib/types';

interface PhilosophyPageProps {
  cards: PhilosophyCard[];
  categories: CategoryItem[];
  selected: PhilosophyCard | null;
  setSelected: (card: PhilosophyCard | null) => void;
...
export const PhilosophyPage = ({
  ...
  if (selected) return (
    <div className="page-enter p-6 md:p-10 max-w-4xl mx-auto">
      <BackButton onClick={() => setSelected(null)} label="Philosophy" />
      <div className="relative rounded-2xl overflow-hidden mb-8" style={{ aspectRatio: '1/1', width: '60%', margin: '0 auto 2rem' }}>
        <Image src={selected.imageUrl || FALLBACKS.yoga} alt={selected.title} fill className="object-cover" />
      </div>
      {selected.category && (
        <span style={{ display: 'inline-block', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--terracotta)', background: 'var(--terracotta-pale)', padding: '4px 12px', borderRadius: 20, marginBottom: 12 }}>
          {selected.category}
        </span>
      )}
  ...
  return (
    <div className="page-enter p-6 md:p-10 max-w-6xl mx-auto">
      ...
      {cards.length === 0 ? (
        ...
        <ContentCardGrid items={cards} onSelect={setSelected} fallback={FALLBACKS.yoga} />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
          {groups.map(group => (
            <div key={group.category}>
              ...
              <ContentCardGrid items={group.items} onSelect={setSelected} fallback={FALLBACKS.yoga} />
            </div>
          ))}
        </div>
      )}
